...
export default function ProcurePage() {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (!response.ok) throw new Error("Status connection drop out.");
  ...
  // LINE 57 FIXED: Resilient Client-Side Header Parsing Engine
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!gatewayResponse.ok) {
          ...
          throw new Error(errorPayload.error || "Gateway ingress rejected matrix data.");
        }
