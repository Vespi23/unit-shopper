...
const UNIT_REGEX = {
  fl_oz:
    /(\d+(?:\.\d+)?)\s?(?:fl\.?\s?oz\.?|fluid\s?ounces?|fl\.?\s?ounces?)\b/i,
  ...
  sq_ft:
    /(\d+(?:\.\d+)?)\s?(?:sq\s?ft|sq\.\s?ft|square\s?foot|square\s?feet)\b/i,
  loads: /(\d+)\s?(?:load|loads)\b/i,
  rolls:
    /(\d+)\s?(?:(?:mega|family|regular|double|triple|huge|super|giant|big|large|bulk)\s+){0,3}(?:roll|rolls)\b/i,
...
export function parseUnit(title: string): UnitInfo | null {
  ...
  const mixedWeightMatch = lowerTitle.match(
    /(\d+(?:\.\d+)?)\s?lbs?\s?(\d+(?:\.\d+)?)\s?oz/i
  )
  if (mixedWeightMatch) {
    ...
    const isCountableItem =
      /trash\s?bag|garbage\s?bag|paper\s?plate|wipe|diaper|tissue|napkin|swiffer|pods?|k-cup/i.test(
        lowerTitle
      )
    if (isCountableItem) {
  ...
  const isExplicitTotal = new RegExp(
